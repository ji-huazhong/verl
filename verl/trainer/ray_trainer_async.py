...
class GenerationBatchFuture:
    ...
    def __init__(self, epoch, batch, gen_batch_output):
        ...
        Args:
            ...
            gen_batch_output: Generated sequences from the main model (DataProtoFuture)
        """
        self.epoch = epoch
        self.batch = batch
    ...
    def get(self):
        ...
        if hasattr(self.gen_batch_output, "get"):
            gen_batch_result = self.gen_batch_output.get() # ray actor handle提供的get方法
        else:
...
class OneStepAsyncRayTrainer(RayPPOTrainer):
    def __init__(
        self,
        config,
        tokenizer,
    ...
    def _async_gen_next_batch(self, continuous_iterator):
        """
        Call model weight synchronization and asynchronous sequence generation.
        """
        try:
        ...
        # sync weights from actor to rollout，每次rollout前先触发权重同步（效率很高）
        self.sync_rollout_weights()
        # async generation
        gen_batch_output = self.rollout_wg.async_generate_sequences(gen_batch)
        # gen_batch_output是DataProtoFuture类型，有get方法
        """
        class DataProtoFuture:
            def get(self):
                output = ray.get(self.futures)
                output = self.collect_nf(output)
