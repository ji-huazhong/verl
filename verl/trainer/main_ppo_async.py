...
from .ray_trainer_async import OneStepAsyncRayTrainer


@hydra.main(config_path="config", config_name="one_step_async_ppo_trainer", version_base=None)
def main(config):
    run_ppo(config)
...
class TaskRunner:
    def run(self, config):
        ...
        from .ray_trainer_async import ResourcePoolManager, Role
        ...
        trainer = OneStepAsyncRayTrainer(
            config=config,
            tokenizer=tokenizer,
            processor=processor,
