...
from verl.models.mcore.qwen3_8_next.ops.ple import Qwen38NextPLE, current_ple_batch
...
class Qwen38NextHCHeadContraction(MegatronModule):
    ...
    def __init__(self, config: TransformerConfig, hc_count: int | None = None, *, hidden_size=None, eps=None):
        super().__init__(config)
        if hidden_size is not None and hidden_size != config.hidden_size:
            raise ValueError("HC head contraction hidden size must match the language model")
        if eps is not None and eps != config.layernorm_epsilon:
            raise ValueError("HC head contraction epsilon must match the language model")
        self.n = hc_count if hc_count is not None else config.num_residual_streams
        self.hidden_size = config.hidden_size
        self.norm_eps = config.layernorm_epsilon
