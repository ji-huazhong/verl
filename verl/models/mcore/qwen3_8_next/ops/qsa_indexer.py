...
from .sequence import apply_indexer_rope, packed_token_segments
...
class PackedBlockLayout:
    ...
    def __init__(self, cu_seqlens: Tensor, positions: Tensor, compress_ratio: int):
        ...
        self.sequence_starts = seq_token_start

        total = positions.numel()
        seg, expected_positions = packed_token_segments(cu, total)
        if not torch.equal(positions, expected_positions):
            raise ValueError("QSA positions must restart at each packed sequence boundary")
...
class Qwen38NextQSAIndexer(MegatronModule):
    ...
    def project_qk(
        ...
        if rotary_pos_emb is not None:
            # vLLM assigns a compressed key the mRoPE position of the FIRST
            # token in that request-local block. Do not index a packed angle
            # buffer by request-local positions from a different sequence.
            if layout is None:
                first_tokens = torch.arange(block_k.shape[0], device=positions.device) * self.compress_ratio
            else:
            ...
            block_k = apply_indexer_rope(block_k.unsqueeze(1), rotary_pos_emb[first_tokens]).squeeze(1)
    ...
    def forward(
        ...
        q, block_k = self.project_qk(hidden_states, positions, layout=layout, rotary_pos_emb=rotary_pos_emb)
