...
# Adapted from radixark/miles PR #2777, revision 6aea72c18bd92c35e53e7d02699a01e02de8993f.
# Modifications: use the verl package namespace. See THIRD_PARTY.md.
...
from verl.models.mcore.qwen3_8_next.ops.kernel.qsa_sparse_attn import qsa_sparse_attention_triton
from verl.models.mcore.qwen3_8_next.ops.qsa_indexer import PackedBlockLayout, Qwen38NextQSAIndexer
...
class Qwen38NextAttention(Qwen3VLSelfAttention):
    ...
    def _packed_positions(cu_seqlens: Tensor, total: int) -> Tensor:
        ...
        return packed_token_segments(cu_seqlens, total)[1]

    def forward(self, hidden_states: Tensor, *args, **kwargs):
        packed = kwargs.get("packed_seq_params")
        if hidden_states.shape[1] != 1:
            raise NotImplementedError("Pack independent sequences into THD; QSA requires batch dimension 1")
        rotary = kwargs.get("rotary_pos_emb")
        if isinstance(rotary, tuple):
            rotary = rotary[0]
        if rotary is None:
            raise ValueError("QSA indexer requires the main attention's composed mRoPE angles")
        if packed is not None:
            padded = getattr(packed, "cu_seqlens_q_padded", None)
            if padded is not None and not torch.equal(padded, packed.cu_seqlens_q):
                raise NotImplementedError("QSA padding gaps need explicit logical-to-physical token mapping")
        ...
        with torch.no_grad():
            selection = self.indexer(
                indexer_states[:, 0], positions, cu_seqlens=self._qsa_cu_seqlens, rotary_pos_emb=rotary
            )
