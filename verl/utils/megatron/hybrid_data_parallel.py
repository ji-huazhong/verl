...
def generate_hdp_group_from_batch(
    ...
    if total_ranks > cp_size:
        raise RuntimeError(
            f"HDP group generated failed: total ranks {total_ranks}  exceeds cp_size {cp_size}. "
...
def pack_sequences_into_buckets(seqlen_list: list[int], max_token_len: int) -> list[list[int]]:
    ...
    has a capacity of cp_size ranks. Qwe greedily pack sequences without exceeding cp_size.
    """
    cp_size = mpu.get_context_parallel_world_size()
    per_rank_cap = max_token_len / cp_size
