...
def test_exporter_keeps_individual_expert_weight_shape_resident():
    ...
    exported = dict(Int4WeightExporter(group_size=32).process_weights_iterator([(f"{base_name}.weight", weight)]))
...
def test_exporter_diagnostics_aggregate_without_per_tensor_host_reads(monkeypatch, caplog):
    ...
    weights = [
        (f"model.layers.0.mlp.experts.{expert_id}.gate_proj.weight", torch.ones(2, 128)) for expert_id in range(2)
    ]
