...
The worker therefore logs end-to-end `reload receive/load`, local vLLM `load_weights`, ownership-copy time, input/retained bytes, and `WNA16 layerwise reload finalized` separately. Set `VERL_INT4_QAT_RELOAD_PROFILE=1` for an H20 benchmark: vLLM workers normally suppress INFO messages, so this switch emits only these profiling records at WARNING level. In the controlled Qwen3-30B 4×H20 baseline, each rollout rank received 55,731 tensors / 16.79 GiB in 22.472–23.439 s; vLLM loading took 6.766–7.152 s, retained ownership copying 0.051–0.119 s (0.40 GiB), and WNA16 finalization 0.060–0.070 s. Thus the remaining 15–16 s is exporter/IPC arrival time, rather than a reason to change the Marlin format. An eight-group-per-program pack kernel is bit-exact for both individual Qwen3 and fused Qwen3.5 expert tensors, but its controlled post-step H20 delta was only -0.7%; it is not claimed as a throughput solution. The next measurable target is the 18,432 individual expert tensor invocations in the actor exporter, not WNA16/Marlin finalization. A direct trainer-to-Marlin packet format could avoid some online layout conversion, but is deliberately not part of this first vLLM path: it would tightly couple Megatron export to vLLM's private packed layout and make backend/version upgrades unsafe.
