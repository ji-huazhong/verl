...
class AnalyseCommunicationMsg(ProfilingConfig):
    ...
    def get_comm_group(self, hcom_info, comm_group_list):
        for index, (name, info) in enumerate(hcom_info.items()):
            if 'hcom' not in name:
                continue
    ...
    def analyse_parallel_comm(self):
        ...
        if self.search_cfg.tp > 1:
            ...
            reduceScatter_num, reduceScatter_index = 0, 0
            for stream_info in parallel_comm_group.tp_comm_group.stream_list:
                if 'reduceScatter' in stream_info.name:
                    reduceScatter_num += 1
            for stream_info in parallel_comm_group.tp_comm_group.stream_list:
                if (logits_info_flag and reduceScatter_num % 3 == 1) or reduceScatter_index >= reduceScatter_num:
                    if 'reduceScatter' in stream_info.name:
                ...
                if 'reduceScatter' in stream_info.name:
                    reduceScatter_index += 1
                if ~logits_info_flag and (reduceScatter_index == reduceScatter_num / 3 or \
                                          reduceScatter_index == (reduceScatter_num - 1) / 3):
        ...
        if self.search_cfg.dp * self.search_cfg.cp > 1:
            if comm_group_index < len(comm_group_orderly_list) and comm_group_orderly_list[comm_group_index].stream_num_without_allreduce <= 1:
            ...
            comm_group_index += 1
            if parallel_comm_group.dp_comm_group is not None and parallel_comm_group.dp_comm_group.stream_list is not None:
    ...
    def _megatron_ep_adaptation(self, stream_list):
        for index, stream_info in enumerate(stream_list):
